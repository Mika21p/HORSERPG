      ...
      <section className="mt-8 grid gap-6 xl:grid-cols-[minmax(19rem,0.8fr)_minmax(0,1.2fr)]">
        <section className="scroll-mt-32 h-fit rounded-xl border border-stone-800 bg-stone-900 p-6" id="forced">
          ...
          <ActionForm action={createGmRetirementRequest} className="mt-5 space-y-4" confirmation="确认创建强制退役申请吗？Horse 会进入退役处理中，但不会立刻退役或释放奖金。" pendingLabel="正在创建…" submitLabel="创建强制退役申请">
            <label className="admin-label">Horse
              <select className="admin-input" defaultValue="" name="horse_id" required>
                <option disabled value="">选择 ACTIVE Horse</option>
                {eligibleForceHorses.map((horse) => <option key={horse.id} value={horse.id}>{formatHorseName(horse)} · #{horse.horse_number} · Owner：{ownerNameById.get(horse.owner_id ?? "") ?? "—"} · 当前 G1 {g1WinsByHorseId.get(horse.id) ?? 0} 胜</option>)}
              </select>
            </label>
                ...
                <article className="rounded-xl border border-stone-800 bg-stone-900 p-5 sm:p-6" key={request.id}>
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-stone-400">Horse #{horse?.horse_number ?? "—"} · {horse ? `${gameState ? gameState.current_wp_year - horse.birth_year : "—"} 岁 · ${formatHorseLifeStage(horse.life_stage)}` : "资料不可用"} · 当前 Owner：{ownerNameById.get(request.owner_id) ?? "—"}</p>
                    </div>
                  ...
                  <div className="mt-5 grid gap-4 lg:grid-cols-2">
                    <section className="rounded-lg border border-stone-800 bg-stone-950/60 p-4"><p className="text-xs font-semibold tracking-[0.14em] text-stone-500">申请事实</p><p className="mt-2 font-medium text-stone-100">{formatHorseRetirementRequestKind(request.request_kind)}</p><p className="mt-2 text-sm text-stone-400">提交于 {formatDateTime(request.requested_at)}</p>{request.player_note && <p className="mt-3 text-sm leading-6 text-stone-300">PLAYER Note：{request.player_note}</p>}{request.gm_reason && <p className="mt-3 text-sm leading-6 text-stone-300">GM 原因：{request.gm_reason}</p>}</section>
                  ...
                  <section className="mt-4 rounded-lg border border-stone-800 bg-stone-950/60 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold tracking-[0.14em] text-stone-500">待释放奖金预览</p><p className="mt-2 text-sm text-stone-400">按 prize_receivables.owner_id 分组，而非当前 Horse Owner。0 笔也可以确认退役。</p></div><p className="break-all font-mono text-lg font-semibold text-amber-100">{formatGameMoney(sumMoney(pendingPrizes))}</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{Array.from(prizeByOwner.entries()).map(([ownerId, rows]) => <div className="rounded border border-stone-800 p-3" key={ownerId}><p className="text-xs text-stone-500">历史 Owner</p><p className="mt-1 text-sm text-stone-100">{ownerNameById.get(ownerId) ?? "Owner"}</p><p className="mt-2 font-mono text-amber-100">{formatGameMoney(sumMoney(rows))} · {rows.length} 笔</p></div>)}{!pendingPrizes.length && <p className="text-sm text-stone-500">无待释放奖金；确认流程仍会完成 Horse 退役并保留完整审计。</p>}</div></section>
                  <div className="mt-5 grid gap-4 border-t border-stone-800 pt-5 lg:grid-cols-2"><ActionForm action={reject} className="rounded-lg border border-red-400/25 bg-red-400/5 p-4" confirmation="确认拒绝这条退役申请吗？Horse 会恢复为现役状态，且该拒绝原因将保留在记录中。" pendingLabel="正在拒绝…" submitLabel="拒绝申请" variant="danger"><label className="admin-label">拒绝原因<textarea className="admin-input min-h-20" name="reason" required /></label></ActionForm>{futureEntries.length ? <div className="rounded-lg border border-stone-700 bg-stone-950/60 p-4"><p className="font-medium text-stone-300">暂不能确认退役</p><p className="mt-2 text-sm leading-6 text-stone-500">先处理上述未来已确认赛程。数据库也会在确认时再次验证，以防并发变化。</p></div> : <ActionForm action={confirm} className="rounded-lg border border-amber-300/35 bg-amber-300/5 p-4" confirmation="最终确认：此操作会永久将 Horse 设为已退役，并在同一数据库事务中释放该 Horse 所有待释放奖金。确认后不能通过普通流程撤销。" pendingLabel="正在确认并结算…" submitLabel="确认退役并结算奖金" variant="danger"><p className="text-sm leading-6 text-amber-100">确认成功后请以刷新后的实际结果为准；数据库采用幂等保护，不会重复释放奖金。</p></ActionForm>}</div>
                </article>
              );
            })}
