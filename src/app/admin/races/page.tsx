...
function horseName(horse: { translated_name: string | null; foal_name: string } | undefined) {
  return horse?.translated_name || horse?.foal_name || "马匹";
}
  ...
  return (
    <main className="page-wrap">
      ...
      <section className="scroll-mt-32 mt-8" id="pending">
        <div className="flex items-end justify-between gap-4"><div><h2 className="text-2xl font-semibold text-amber-200">待审核报名</h2><p className="mt-2 text-sm text-stone-400">玩家原始请求永远只读；GM 可独立编辑最终时间、比赛、骑手、跑法与内部备注。</p></div><span className="font-mono text-sm text-stone-500">{pendingRequests.length} 条</span></div>
          ...
          {pendingRequests.map((request) => {
            ...
            return (
              <article className="rounded-xl border border-stone-800 bg-stone-900 p-6" key={request.id}>
                <div className="flex flex-col gap-3 border-b border-stone-800 pb-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-semibold text-stone-100">{horseName(horse)} <span className="font-mono text-sm text-stone-500">#{horse?.horse_number ?? "—"}</span></p><p className="mt-1 text-sm text-stone-400">马主：{ownerNames.get(request.owner_id) ?? "未知马主"} · 提交于 {formatDateTime(request.created_at)}</p></div><span className="rounded-full border border-amber-300/40 bg-amber-300/10 px-3 py-1 text-xs font-semibold text-amber-100">{formatRaceEntryRequestStatus(request.status)}</span></div>
                <div className="mt-6 grid gap-6 xl:grid-cols-2">
                  <section className="rounded-xl border border-stone-800 bg-stone-950/60 p-5"><p className="text-xs font-semibold tracking-[0.16em] text-stone-500">玩家原始请求 · 只读</p><p className="mt-4 text-lg font-semibold text-stone-100">{formatWpTime(request.requested_wp_year, request.requested_wp_month, request.requested_wp_week)}</p><p className="mt-2 font-medium text-amber-100">{requestedRaceName(request, catalogNames)}</p><dl className="mt-5 grid gap-3 text-sm"><div><dt className="text-stone-500">希望骑手</dt><dd className="mt-1 text-stone-200">{request.requested_jockey || "未指定"}</dd></div><div><dt className="text-stone-500">希望跑法</dt><dd className="mt-1 text-stone-200">{request.requested_running_style || "未指定"}</dd></div><div><dt className="text-stone-500">玩家备注</dt><dd className="mt-1 whitespace-pre-wrap leading-6 text-stone-200">{request.player_note || "—"}</dd></div></dl>{activeInjuries.length > 0 && <div className="mt-5 rounded-lg border border-red-400/35 bg-red-400/5 p-3 text-sm leading-6 text-red-100"><p className="font-semibold">当前有效伤病</p>{activeInjuries.map((injury, index) => <p className="mt-2" key={`${injury.wp_start_year}-${injury.wp_start_month}-${injury.wp_start_week}-${index}`}>{formatWpTime(injury.wp_start_year, injury.wp_start_month, injury.wp_start_week)} 至 {formatWpTime(injury.wp_end_year, injury.wp_end_month, injury.wp_end_week)}。结束周仍不可参赛，从下一 WP 周开始可报名。</p>)}</div>}</section>
                  <section className="rounded-xl border border-emerald-400/30 bg-emerald-400/5 p-5"><p className="text-xs font-semibold tracking-[0.16em] text-emerald-200">GM 最终确认 · 可编辑</p><p className="mt-2 text-sm leading-6 text-stone-400">可以改变周次或比赛；骑手与跑法可留空。数据库会重新核验马匹状态、马主、伤病、时间和同周冲突。</p><div className="mt-5"><RaceScheduleForm action={confirm} catalogs={activeCatalogs} confirmation="确认以当前最终事实安排赛程吗？若与玩家请求不同，将以此最终事实公开。" defaultValues={{ wpYear: request.requested_wp_year, wpMonth: request.requested_wp_month, wpWeek: request.requested_wp_week, raceKind: request.requested_race_kind, raceCatalogId: request.requested_race_catalog_id, raceLabel: request.requested_race_label, jockey: request.requested_jockey, runningStyle: request.requested_running_style }} mode="GM_CONFIRM" pendingLabel="正在确认…" submitLabel="确认赛程" /></div></section>
                </div>
                <details className="mt-5 rounded-lg border border-red-400/25 bg-red-400/5 p-4"><summary className="cursor-pointer text-sm font-semibold text-red-100">拒绝这条报名</summary><p className="mt-2 text-sm leading-6 text-stone-300">原因可留空；如填写，会向该玩家 展示。</p><ActionForm action={reject} className="mt-4 space-y-3" confirmation="确定拒绝这条 待处理报名吗？" pendingLabel="正在拒绝…" submitLabel="拒绝报名" variant="danger"><label className="admin-label">拒绝原因（可选）<textarea className="admin-input min-h-20" name="rejection_reason" placeholder="例如：赛程已调整" /></label></ActionForm></details>
              </article>
            );
          })}
      ...
      <section className="scroll-mt-32 mt-10 grid gap-8 xl:grid-cols-[minmax(0,0.9fr)_minmax(24rem,1.1fr)]" id="direct">
        ...
        <section className="rounded-xl border border-stone-800 bg-stone-900 p-6"><h2 className="text-xl font-semibold text-amber-200">GM 直接安排赛程</h2><p className="mt-2 text-sm leading-6 text-stone-400">这是独立于玩家申请的权威安排。不会创建伪造申请，也不会让客户端传入马主。</p>{gameState ? <div className="mt-5"><RaceScheduleForm action={createDirectRaceEntry} catalogs={activeCatalogs} confirmation="确认直接安排这条最终赛程吗？相同最终事实的重试会返回原记录。" defaultValues={{ wpYear: gameState.current_wp_year, wpMonth: gameState.current_wp_month, wpWeek: gameState.current_wp_week, raceKind: "CATALOG" }} horses={horseOptions} mode="GM_DIRECT" pendingLabel="正在安排…" submitLabel="直接安排赛程" /></div> : <p className="mt-5 text-sm text-amber-100">请先设置游戏时间。</p>}</section>
        <section className="rounded-xl border border-stone-800 bg-stone-900 p-6"><h2 className="text-xl font-semibold text-amber-200">创建比赛目录</h2><p className="mt-2 text-sm leading-6 text-stone-400">目录默认月 / 周只用于报名表单建议，不约束 GM 最终确认，也不会修改既有赛程。</p><ActionForm action={createRaceCatalog} className="mt-5 space-y-4" pendingLabel="正在创建…" submitLabel="创建固定比赛"><label className="admin-label">比赛名称<input className="admin-input" name="name" required /></label><div className="grid gap-4 sm:grid-cols-3"><label className="admin-label">分级<select className="admin-input" defaultValue="OP" name="grade"><option value="OP">OP</option><option value="G3">G3</option><option value="G2">G2</option><option value="G1">G1</option></select></label><label className="admin-label">默认月<select className="admin-input" defaultValue="4" name="default_wp_month">{Array.from({ length: 12 }, (_, index) => index + 1).map((month) => <option key={month} value={month}>{month} 月</option>)}</select></label><label className="admin-label">默认周<select className="admin-input" defaultValue="1" name="default_wp_week">{Array.from({ length: 5 }, (_, index) => index + 1).map((week) => <option key={week} value={week}>第 {week} 周</option>)}</select></label></div><label className="flex items-center gap-2 text-sm text-stone-300"><input defaultChecked name="is_active" type="checkbox" />立即启用</label></ActionForm></section>
      </section>

      <details className="mt-10 rounded-xl border border-stone-800 bg-stone-900 p-6" open><summary className="cursor-pointer text-xl font-semibold text-amber-200">已确认赛程（GM 内部）</summary><p className="mt-2 text-sm text-stone-400">此区域包含报名来源与 GM 备注；玩家页面不会请求这些字段。</p><div className="mt-5 grid gap-4 lg:grid-cols-2">{(entries ?? []).map((entry) => { const sourceRequest = entry.request_id ? requestsById.get(entry.request_id) : null; return <article className="rounded-lg border border-stone-800 bg-stone-950/60 p-5" key={entry.id}><div className="flex flex-col gap-3 sm:flex-row sm:justify-between"><div><p className="font-semibold text-stone-100">{horseName(horsesById.get(entry.horse_id))}</p><p className="mt-1 text-sm text-stone-400">马主：{ownerNames.get(entry.owner_id) ?? "未知马主"}</p></div><p className="font-mono text-sm text-amber-200">{formatWpTime(entry.wp_year, entry.wp_month, entry.wp_week)}</p></div><p className="mt-4 font-medium text-amber-100">{raceName(entry, catalogNames)}</p><p className="mt-2 text-sm text-stone-400">骑手：{entry.jockey || "未指定"} · 跑法：{entry.running_style || "未指定"}</p><p className="mt-3 text-xs font-semibold tracking-wide text-stone-500">来源：{entry.request_id ? "玩家申请" : "GM 直接安排"}</p>{sourceRequest && <p className="mt-1 text-xs text-stone-500">原请求：{formatWpTime(sourceRequest.requested_wp_year, sourceRequest.requested_wp_month, sourceRequest.requested_wp_week)} · {requestedRaceName(sourceRequest, catalogNames)}</p>}{entry.gm_note && <p className="mt-3 rounded border border-stone-800 bg-stone-900 p-3 text-sm leading-6 text-stone-300">GM 备注：{entry.gm_note}</p>}<p className="mt-3 text-xs text-stone-500">确认于 {formatDateTime(entry.confirmed_at)}</p></article>; })}{!entries?.length && <p className="rounded-lg border border-stone-800 bg-stone-950/60 p-5 text-sm text-stone-500">暂无已确认赛程。</p>}</div></details>

      <details className="mt-6 rounded-xl border border-stone-800 bg-stone-900 p-6"><summary className="cursor-pointer text-xl font-semibold text-amber-200">比赛目录管理</summary><p className="mt-2 text-sm text-stone-400">不提供物理删除。需要停止新选择时，将目录设为停用；历史申请与已确认赛程仍保持完整。</p><div className="mt-5 grid gap-4 lg:grid-cols-2">{(catalogs ?? []).map((catalog) => { const save = updateRaceCatalog.bind(null, catalog.id); return <article className="rounded-lg border border-stone-800 bg-stone-950/60 p-5" key={catalog.id}><ActionForm action={save} className="space-y-3" pendingLabel="正在保存…" submitLabel="保存目录"><label className="admin-label">比赛名称<input className="admin-input" defaultValue={catalog.name} name="name" required /></label><div className="grid gap-3 sm:grid-cols-3"><label className="admin-label">分级<select className="admin-input" defaultValue={catalog.grade} name="grade"><option value="OP">OP</option><option value="G3">G3</option><option value="G2">G2</option><option value="G1">G1</option></select></label><label className="admin-label">默认月<select className="admin-input" defaultValue={catalog.default_wp_month} name="default_wp_month">{Array.from({ length: 12 }, (_, index) => index + 1).map((month) => <option key={month} value={month}>{month} 月</option>)}</select></label><label className="admin-label">默认周<select className="admin-input" defaultValue={catalog.default_wp_week} name="default_wp_week">{Array.from({ length: 5 }, (_, index) => index + 1).map((week) => <option key={week} value={week}>第 {week} 周</option>)}</select></label></div><label className="flex items-center gap-2 text-sm text-stone-300"><input defaultChecked={catalog.is_active} name="is_active" type="checkbox" />允许新报名与新确认选择</label></ActionForm></article>; })}{!catalogs?.length && <p className="rounded-lg border border-stone-800 bg-stone-950/60 p-5 text-sm text-stone-500">尚无比赛目录。</p>}</div></details>
    </main>
  );
}
