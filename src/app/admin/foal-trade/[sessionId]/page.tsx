    ...
    <main className="page-wrap">
        ...
        <div className="rounded-xl border border-stone-800 bg-stone-900 p-6">
          ...
          <ActionForm action={updateFoalTradeSessionStatus.bind(null, session.id)} className="mt-5 flex flex-col gap-3 sm:flex-row" pendingLabel="正在保存…" submitLabel="保存状态">
            <select className="admin-input mt-0 flex-1" defaultValue={session.status} name="status">
              <option value="DRAFT">草稿 DRAFT</option><option value="OPEN">开放 OPEN</option><option value="CLOSED">关闭 CLOSED</option><option value="REVIEWING">审核 REVIEWING</option>
            </select>
          </ActionForm>
          <p className="mt-3 text-xs leading-5 text-stone-500">`SETTLED` 由所有 Lot 结算完成后自动设定，不在此手动选择。</p>
        </div>
      ...
      {canConfigure && (
        <section className="mt-6 rounded-xl border border-stone-800 bg-stone-900 p-6">
          <h2 className="text-xl font-semibold text-amber-200">加入 Lot</h2>
          <p className="mt-2 text-sm text-stone-400">仅列出本届出生批次、未归属、仍处于 FOAL 且尚未参加任何庭先的 Horse。数据库会再次验证这些条件。</p>
          <ActionForm action={createFoalTradeLot.bind(null, session.id)} className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_14rem_auto]" pendingLabel="正在创建…" submitLabel="创建 Lot">
            <select className="admin-input mt-0" name="horse_id" required><option value="">选择符合条件的 Horse</option>{availableHorses.map((horse) => <option key={horse.id} value={horse.id}>#{horse.horse_number} · {horse.translated_name || horse.foal_name}</option>)}</select>
      ...
      <section className="scroll-mt-32 mt-8 grid gap-6" id="lots">
          ...
          return (
            <article className="rounded-xl border border-stone-800 bg-stone-900 p-6" key={lot.id}>
              <div className="flex flex-col justify-between gap-5 lg:flex-row">
                <div>
                  <p className="font-mono text-sm font-semibold text-amber-300">LOT · HORSE #{horse?.horse_number ?? "—"}</p>
                  <h2 className="mt-2 text-2xl font-semibold">{horse?.translated_name || horse?.foal_name || "Horse 资料不可用"}</h2>
                  <p className="mt-2 text-sm text-stone-400">{horse ? `${horse.sex} / ${horse.coat_color} · 父：${horse.sire_name} · 父系：${horse.sire_line} · 母父：${horse.broodmare_sire_name}` : "—"}</p>
                  <p className="mt-2 text-sm text-stone-400">Factors：{horseFactors.map((factor) => `${factor.factor_kind} ${factor.factor_name}`).join(" · ") || "—"}</p>
                </div>
              ...
              {settlement ? (
                ...
                <div className="mt-6 grid gap-6 border-t border-stone-800 pt-6 xl:grid-cols-2">
                    ...
                    <div className="mt-3 grid gap-3">
                      {lotInquiries.map((inquiry) => (
                        <div className="rounded-lg border border-stone-800 bg-stone-950/60 p-4" key={inquiry.id}>
                          <p className="text-sm font-medium text-stone-100">{ownerNameById.get(inquiry.owner_id) ?? "Owner"}</p>
                          <p className="mt-1 text-xs text-stone-500">请求：{formatDateTime(inquiry.requested_at)} · {inquiry.status}</p>
                  ...
                  <section>
                    ...
                    <div className="mt-3 overflow-hidden rounded-lg border border-stone-800"><table className="w-full text-left text-sm"><thead className="bg-stone-950 text-stone-500"><tr><th className="px-3 py-2">Owner</th><th className="px-3 py-2">金额</th><th className="px-3 py-2">优先时间</th><th className="px-3 py-2">状态</th></tr></thead><tbody className="divide-y divide-stone-800">{lotOffers.map((offer) => <tr key={offer.id}><td className="px-3 py-2 text-stone-200">{ownerNameById.get(offer.owner_id) ?? "Owner"}</td><td className="px-3 py-2 font-mono text-amber-200">{formatGameMoney(offer.amount)}</td><td className="px-3 py-2 text-stone-400">{formatDateTime(offer.priority_at)}</td><td className="px-3 py-2 text-stone-400">{offer.status}</td></tr>)}{!lotOffers.length && <tr><td className="px-3 py-4 text-stone-500" colSpan={4}>尚无秘密报价。</td></tr>}</tbody></table></div>
                    <div className="mt-4 rounded-lg border border-amber-300/20 bg-amber-300/5 p-4 text-sm"><p className="font-semibold text-amber-100">系统推荐</p>{recommended ? <p className="mt-2 text-stone-200">{ownerNameById.get(recommended.owner_id) ?? "Owner"} · <span className="font-mono text-amber-200">{formatGameMoney(recommended.amount)}</span><span className="block mt-1 text-xs text-stone-500">priority_at：{formatDateTime(recommended.priority_at)}</span></p> : <p className="mt-2 text-stone-400">没有有效报价；正常结算会记录为未成交。</p>}</div>
                    <div className="mt-4 flex flex-col gap-3">
                      ...
                      {recommended && activeOffers.length > 1 && (
                        <details className="rounded-lg border border-red-400/30 bg-red-400/5 p-4"><summary className="cursor-pointer text-sm font-semibold text-red-200">更多操作：GM 例外裁定</summary><p className="mt-3 text-sm leading-6 text-stone-300">这是异常路径：必须选择非系统推荐的有效报价，并填写会写入审计记录的理由。</p><ActionForm action={settleFoalTradeLotOverride.bind(null, session.id, lot.id)} className="mt-4 grid gap-3" confirmation="确认执行 GM 例外裁定？此操作会按所选有效报价结算并记录审计。" pendingLabel="正在裁定…" submitLabel="确认例外裁定" variant="danger"><select className="admin-input mt-0" name="selected_offer_id" required><option value="">选择另一条有效报价</option>{activeOffers.filter((offer) => offer.id !== recommended.id).map((offer) => <option key={offer.id} value={offer.id}>{ownerNameById.get(offer.owner_id) ?? "Owner"} · {formatGameMoney(offer.amount)} · {formatDateTime(offer.priority_at)}</option>)}</select><textarea className="admin-input mt-0 min-h-24" name="override_reason" placeholder="必须填写例外裁定理由" required /></ActionForm></details>
                      )}
                    </div>
                  </section>
                </div>
        ...
        {!lots?.length && <p className="text-stone-500">本届尚未加入 Lot。</p>}
      </section>
    </main>
  );
}
