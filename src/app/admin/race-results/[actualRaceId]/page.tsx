...
export default async function ActualRaceDetailPage({ params, searchParams }: PageProps) {
  ...
  function toCandidate(entry: NonNullable<typeof entries>[number]): RaceResultCandidate {
    ...
    return { id: entry.id, horseId: entry.horse_id, horseName: horsesById.get(entry.horse_id)?.translated_name || horsesById.get(entry.horse_id)?.foal_name || "马匹", horseNumber: horsesById.get(entry.horse_id)?.horse_number ?? "—", ownerName: ownerNames.get(entry.owner_id) ?? "未知马主", currentStamina: horsesById.get(entry.horse_id)?.current_stamina ?? null, wpYear: entry.wp_year, wpMonth: entry.wp_month, wpWeek: entry.wp_week, raceKind: entry.race_kind, raceCatalogId: entry.race_catalog_id, raceLabel: entryRaceName(entry), jockey: entry.jockey, runningStyle: entry.running_style, source: entry.request_id ? "玩家申请" : "GM 直接安排", recommendation };
  }
