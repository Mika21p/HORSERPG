...
export default async function AdminRaceResultsPage({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
    <section className="flex flex-col gap-5 border-b border-stone-800 pb-7 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-sm font-semibold tracking-[0.24em] text-amber-300">GM · RACE RESULTS</p><h1 className="mt-3 text-3xl font-semibold tracking-tight">赛果管理</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-stone-400">以 Actual Race 为中心录入 Winning Post 实际赛果。赛前 Confirmed Entry、实际比赛与赛果是三个独立事实；计划与实际可以不同。</p></div>
    ...
    <section className="mt-8 grid gap-6 xl:grid-cols-[minmax(20rem,0.85fr)_minmax(0,1.15fr)]">
      <section className="rounded-xl border border-stone-800 bg-stone-900 p-6"><h2 className="text-xl font-semibold text-amber-200">新建 Actual Race</h2><p className="mt-2 text-sm leading-6 text-stone-400">Actual Race 必须由 GM 明确创建。Catalog 会保存名称和 Grade 历史快照；已停用 Catalog 也可用于历史补录。</p>{gameState ? <div className="mt-5"><ActualRaceForm action={createActualRace} catalogs={catalogOptions} currentWp={{ year: gameState.current_wp_year, month: gameState.current_wp_month, week: gameState.current_wp_week }} pendingLabel="正在创建…" submitLabel="创建实际比赛" /></div> : <p className="mt-5 rounded-lg border border-amber-300/30 bg-amber-300/5 p-4 text-sm text-amber-100">请先初始化 Game State。</p>}</section>
      <section className="grid gap-4 sm:grid-cols-2"><article className="rounded-xl border border-stone-800 bg-stone-900 p-6"><p className="text-sm text-stone-500">Actual Races</p><p className="mt-2 text-3xl font-semibold text-amber-100">{actualRaces?.length ?? 0}</p><p className="mt-2 text-sm leading-6 text-stone-400">每场实际比赛可关联多匹玩家 Horse 的赛果。</p></article><article className="rounded-xl border border-stone-800 bg-stone-900 p-6"><p className="text-sm text-stone-500">待录赛果</p><p className="mt-2 text-3xl font-semibold text-amber-100">{overdueEntries.length}</p><p className="mt-2 text-sm leading-6 text-stone-400">已到当前周但尚无有效 Race Result 的 Confirmed Entries。</p></article><article className="rounded-xl border border-stone-800 bg-stone-900 p-6 sm:col-span-2"><p className="text-sm text-stone-500">已录当前有效赛果</p><p className="mt-2 text-3xl font-semibold text-emerald-200">{(results ?? []).filter((result) => result.status === "CONFIRMED").length}</p><p className="mt-2 text-sm leading-6 text-stone-400">赏金目前仅是 Winning Post 赛果事实，不会改变 Owner 资金或产生奖金应收。</p></article></section>
    </section>

    <section className="mt-10"><div className="flex items-end justify-between gap-4"><div><h2 className="text-2xl font-semibold text-amber-200">Actual Races</h2><p className="mt-2 text-sm text-stone-400">按实际 WP 时间最近优先。进入详情后可批量关联并录入多匹 Horse 的赛果。</p></div><span className="font-mono text-sm text-stone-500">{actualRaces?.length ?? 0} 场</span></div><div className="mt-5 grid gap-4 lg:grid-cols-2">{(actualRaces ?? []).map((race) => <Link className="rounded-xl border border-stone-800 bg-stone-900 p-5 transition hover:border-amber-300/60" href={`/admin/race-results/${race.id}`} key={race.id}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-semibold text-stone-100">{race.race_name} {race.grade && <span className="ml-2 rounded border border-amber-300/35 px-2 py-0.5 text-xs text-amber-100">{formatRaceGrade(race.grade)}</span>}</p><p className="mt-2 text-sm text-stone-400">{formatWpTime(race.wp_year, race.wp_month, race.wp_week)} · {formatRaceKind(race.race_kind)}</p></div><span className="w-fit rounded-full border border-emerald-400/35 px-3 py-1 text-xs text-emerald-100">{resultCountByActualRace.get(race.id) ?? 0} 条有效赛果</span></div><p className="mt-4 text-xs text-stone-500">创建于 {formatDateTime(race.created_at)} · {race.race_catalog_id ? "Catalog 快照" : "非 Catalog"}</p></Link>)}{!actualRaces?.length && <p className="rounded-xl border border-stone-800 bg-stone-900 p-6 text-sm text-stone-500">暂无已记录的实际比赛。</p>}</div></section>

    <section className="mt-10 grid gap-8 xl:grid-cols-2"><section><h2 className="text-xl font-semibold text-amber-200">待录赛果的 Confirmed Entries</h2><p className="mt-2 text-sm text-stone-400">这是提醒而非自动匹配；进入某场 Actual Race 后由 GM 明确选择关联。</p><div className="mt-5 space-y-3">{overdueEntries.slice(0, 10).map((entry) => <article className="rounded-xl border border-stone-800 bg-stone-900 p-4" key={entry.id}><p className="font-semibold text-stone-100">{formatHorseName(horseById.get(entry.horse_id))} <span className="font-mono text-xs text-stone-500">#{horseById.get(entry.horse_id)?.horse_number ?? "—"}</span></p><p className="mt-1 text-sm text-stone-400">Owner：{ownerNameById.get(entry.owner_id) ?? "Owner"} · {formatWpTime(entry.wp_year, entry.wp_month, entry.wp_week)}</p><p className="mt-2 text-sm text-amber-100">{entry.race_label || (entry.race_catalog_id ? catalogNameById.get(entry.race_catalog_id) ?? "固定比赛" : formatRaceKind(entry.race_kind))}</p></article>)}{!overdueEntries.length && <p className="rounded-xl border border-stone-800 bg-stone-900 p-6 text-sm text-stone-500">当前没有待录的历史赛果。</p>}</div></section><section><h2 className="text-xl font-semibold text-amber-200">最近已录赛果</h2><p className="mt-2 text-sm text-stone-400">当前有效赛果与作废历史在 Actual Race 详情中分别呈现。</p><div className="mt-5 space-y-3">{(results ?? []).filter((result) => result.status === "CONFIRMED").slice(0, 10).map((result) => <Link className="block rounded-xl border border-stone-800 bg-stone-900 p-4 hover:border-amber-300/60" href={`/admin/race-results/${result.actual_race_id}`} key={result.id}><p className="font-semibold text-stone-100">{formatHorseName(horseById.get(result.horse_id))} · {result.finish_position} 着 · {result.prize_amount}</p><p className="mt-1 text-sm text-stone-400">{formatDateTime(result.recorded_at)}</p></Link>)}{!(results ?? []).some((result) => result.status === "CONFIRMED") && <p className="rounded-xl border border-stone-800 bg-stone-900 p-6 text-sm text-stone-500">暂无已录赛果。</p>}</div></section></section>
  </main>;
}
