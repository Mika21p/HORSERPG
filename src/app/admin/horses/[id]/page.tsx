...
export default async function AdminHorseDetailPage({ params, searchParams }: PageProps) {
  ...
  const sourceLabel = (source: string | null) => source === "INTERNAL" ? "内部马匹" : source === "REFERENCE" ? "外部资料" : source === "MANUAL" ? "手动" : "历史文本";
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-6 py-10">
      ...
      <section className="mt-8 rounded-xl border border-stone-800 bg-stone-900 p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-stone-400">新结构化幼驹保留创建时的父母快照；链接只用于导航，不会使用父母当前资料替换本马的历史名称。</p>
      ...
      <section className="mt-8 rounded-xl border border-stone-800 bg-stone-900 p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-stone-400">仅已退役的牡马或牝马可成为候选。候选状态不会改变马匹生命周期，也不会使用繁殖中阶段。</p>
      ...
      <section className="mt-8 rounded-xl border border-stone-800 bg-stone-900 p-6">
        <h2 className="text-xl font-semibold text-amber-200">血统因子</h2>
        <p className="mt-2 text-sm text-stone-400">父系 {sireCount}/2 · 母系 {mareCount}/2。数据库同时强制该上限。</p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          {(factors ?? []).map((factor) => (
            <div className="flex items-center justify-between rounded-lg border border-stone-800 bg-stone-950 px-4 py-3" key={factor.id}>
              <span><span className="mr-2 text-xs font-semibold text-amber-300">{factor.factor_kind === "SIRE" ? "父系" : factor.factor_kind === "MARE" ? "母系" : "未知"}</span>{factor.factor_name}</span>
        ...
        <form action={addHorseFactor.bind(null, horse.id)} className="mt-6 grid gap-3 border-t border-stone-800 pt-5 sm:grid-cols-[10rem_minmax(0,1fr)_auto]">
          <select className="admin-input" defaultValue="SIRE" name="factor_kind"><option value="SIRE">父系</option><option value="MARE">母系</option></select>
          <input className="admin-input" name="factor_name" placeholder="因子名称" required />
          <button className="admin-button">新增因子</button>
        </form>
      </section>
    </main>
  );
