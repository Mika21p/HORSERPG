  ...
  return (
    <main className="mx-auto grid w-full max-w-6xl gap-8 px-6 py-10 lg:grid-cols-[minmax(0,1fr)_24rem]">
      <section>
        ...
        <p className="mt-3 text-stone-400">本页只能创建 PLAYER；GM 只可通过一次性开发者 Bootstrap 工具初始化。</p>
        <Notice message={notice} />
        <div className="mt-6 overflow-hidden rounded-xl border border-stone-800">
          <table className="w-full text-left text-sm">
            <thead className="bg-stone-900 text-stone-400"><tr><th className="px-4 py-3">角色</th><th className="px-4 py-3">显示名</th><th className="px-4 py-3">登录 Email</th><th className="px-4 py-3">Owner</th><th className="px-4 py-3" /></tr></thead>
            <tbody className="divide-y divide-stone-800 bg-stone-950/50">
              {(profiles ?? []).map((profile) => (
                <tr key={profile.id}>
                  <td className="px-4 py-3 text-amber-200">{profile.role}</td>
              ...
              {!profiles?.length && <tr><td className="px-4 py-8 text-stone-500" colSpan={5}>尚无 User Profile。</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
      <section className="h-fit rounded-xl border border-stone-800 bg-stone-900 p-5">
        <h2 className="font-semibold text-amber-200">创建 PLAYER</h2>
        <form action={createPlayer} className="mt-5 space-y-4">
          <label className="admin-label" htmlFor="owner_id">绑定空闲 Owner
          ...
          <label className="admin-label" htmlFor="email">Email<input autoComplete="off" className="admin-input" id="email" name="email" required type="email" /></label>
          ...
          {!availableOwners.length && <p className="text-sm text-stone-500">没有空闲 Owner；请先创建 Owner，或检查已有绑定。</p>}
          <button className="admin-button w-full" disabled={!availableOwners.length}>创建 PLAYER</button>
        </form>
      </section>
    </main>
  );
