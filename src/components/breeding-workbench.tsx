  ...
  return (
    <section className="rounded-xl border border-amber-300/35 bg-amber-300/5 p-5 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-semibold tracking-[0.18em] text-amber-300">GM · PEDIGREE REFERENCE</p>
      ...
      <p className="mt-3 text-sm leading-6 text-stone-400">资料库用于输入辅助，不是新建幼驹的硬性前提。修改资料库不会改写已经创建 Horse 的历史血统快照。</p>
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <label className="admin-label">Aliases
        ...
        <label className="admin-label md:col-span-2">GM Notes
...
function FoalCreationPanel({ candidates, references, gameState, onEditReference }: { candidates: BreedingCandidate[]; references: PedigreeReference[]; gameState: GameState; onEditReference: (reference: PedigreeReference) => void }) {
  ...
  const validate = () => {
    if (!horseNumber || !birthYear || !foalName.trim() || !coatColor.trim()) return "请完整填写 Horse Number、WP 出生年份、幼驹阶段名、性别与毛色。";
