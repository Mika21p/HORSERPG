...
export function RaceScheduleForm({
  ...
  const noteLabel = isPlayer ? "PLAYER 备注（可选）" : "GM Note（可选）";
    ...
    >
      ...
      {selectedHorse?.injuries?.length ? (
        <section className="rounded-lg border border-red-400/35 bg-red-400/5 p-4 text-sm leading-6 text-red-100">
          <p className="font-semibold">当前 ACTIVE 伤病提醒</p>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <label className="admin-label">WP Year<input className="admin-input" min="1" name="wp_year" onChange={(event) => setWpYear(event.target.value)} required step="1" type="number" value={wpYear} /></label>
        <label className="admin-label">Month<select className="admin-input" name="wp_month" onChange={(event) => setWpMonth(event.target.value)} value={wpMonth}>{Array.from({ length: 12 }, (_, index) => index + 1).map((month) => <option key={month} value={month}>{month} 月</option>)}</select></label>
        <label className="admin-label">Week<select className="admin-input" name="wp_week" onChange={(event) => setWpWeek(event.target.value)} value={wpWeek}>{Array.from({ length: 5 }, (_, index) => index + 1).map((week) => <option key={week} value={week}>Week {week}</option>)}</select></label>
      </div>
      ...
      {catalogSelected ? (
        <label className="admin-label">
          ...
          <select className="admin-input" name="race_catalog_id" onChange={(event) => applyCatalogDefaults(event.target.value)} required value={catalogId}>
            ...
            {catalogs.map((catalog) => <option key={catalog.id} value={catalog.id}>{catalog.name} · {catalog.grade} · 默认 {catalog.default_wp_month} 月 Week {catalog.default_wp_week}</option>)}
          </select>
        ...
        <label className="admin-label">
          比赛名称 / 说明
          <input className="admin-input" defaultValue={defaultValues.raceLabel ?? ""} name="race_label" placeholder="例如：3岁未胜利、1胜 Class、地方交流赛" required />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="admin-label">{isPlayer ? "希望骑手（可选）" : "Final Jockey（可选）"}<input className="admin-input" name="jockey" onChange={(event) => setJockey(event.target.value)} placeholder="可留空" value={jockey} /></label>
        <label className="admin-label">{isPlayer ? "希望跑法（可选）" : "Final Running Style（可选）"}<select className="admin-input" defaultValue={defaultValues.runningStyle ?? ""} name="running_style">{runningStyles.map((style) => <option key={style || "unspecified"} value={style}>{style || "未指定"}</option>)}</select></label>
      </div>

      <label className="admin-label">{noteLabel}<textarea className="admin-input min-h-24" defaultValue={defaultValues.note ?? ""} name="note" placeholder={isPlayer ? "例如：优先这场，没有就改下周" : "只供 GM 内部参考，不向 PLAYER 公开"} /></label>
    </ActionForm>
  );
}
