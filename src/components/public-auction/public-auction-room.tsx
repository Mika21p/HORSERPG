    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-10">
      <section className="rounded-2xl border border-stone-800 bg-stone-900 p-5 shadow-2xl shadow-black/20 sm:p-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-semibold tracking-[0.24em] text-amber-300">PUBLIC AUCTION</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">{snapshot ? `${snapshot.event.wp_year} 年公开拍卖会` : "年末公开拍卖会"}</h1>
            {snapshot && <p className="mt-2 text-sm text-stone-400">Event 状态：{formatPublicAuctionStatus(snapshot.event.status)}</p>}
          </div>
          <div className={`rounded-lg border px-4 py-3 text-sm ${connectionState === "connected" ? "border-emerald-300/40 bg-emerald-300/5 text-emerald-100" : "border-amber-300/40 bg-amber-300/5 text-amber-100"}`}>
            ...
            {(connectionState === "disconnected" || connectionState === "error") && <p className="mt-1 text-xs">Realtime 只负责通知；已显示的数据库快照会继续保留。</p>}
      ...
      {!snapshot ? (
        ...
        <section className="mt-6 rounded-2xl border border-stone-800 bg-stone-900 p-8 text-center"><h2 className="text-xl font-semibold text-stone-100">等待 GM 展示下一匹幼驹</h2><p className="mt-3 text-sm text-stone-400">未来 Lot 不会在此页面提前显示。</p></section>
      ) : (
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(22rem,0.85fr)]">
          <div className="space-y-6">
            <section className="rounded-2xl border border-stone-800 bg-stone-900 p-5 sm:p-7">
              ...
              {currentLot.horse && <div className="mt-6 grid gap-3 border-t border-stone-800 pt-5 text-sm text-stone-300 sm:grid-cols-2"><p>性别：{currentLot.horse.sex}</p><p>毛色：{currentLot.horse.coat_color}</p><p>父：{currentLot.horse.sire_name}</p><p>父系统：{currentLot.horse.sire_line}</p><p>母父：{currentLot.horse.broodmare_sire_name}</p><p>SIRE factors：{sireFactors.map((factor) => factor.factor_name).join(" · ") || "—"}</p><p className="sm:col-span-2">MARE factors：{mareFactors.map((factor) => factor.factor_name).join(" · ") || "—"}</p></div>}
            </section>
            ...
            <section className="rounded-2xl border border-stone-800 bg-stone-900 p-5 sm:p-7"><h2 className="text-xl font-semibold text-amber-200">本轮公开报价</h2><div className="mt-5 max-h-80 overflow-y-auto rounded-lg border border-stone-800"><table className="w-full table-fixed text-left text-sm"><thead className="sticky top-0 bg-stone-950 text-stone-500"><tr><th className="w-[31%] px-3 py-3">时间</th><th className="w-[35%] px-3 py-3">Owner</th><th className="w-[34%] px-3 py-3 text-right">报价</th></tr></thead><tbody className="divide-y divide-stone-800">{snapshot.bids.map((bid) => <tr className={bid.owner?.id === currentRound?.current_winner_owner_id ? "bg-amber-300/5" : undefined} key={bid.id}><td className="break-words px-3 py-3 text-stone-400">{formatDateTime(bid.accepted_at)}</td><td className="break-words px-3 py-3 text-stone-200">{bid.owner?.display_name ?? "Owner"}</td><td className="break-all px-3 py-3 text-right font-mono text-amber-200">{formatGameMoney(bid.amount)}</td></tr>)}{!snapshot.bids.length && <tr><td className="px-3 py-6 text-center text-stone-500" colSpan={3}>本轮尚无公开报价。</td></tr>}</tbody></table></div></section>
          </div>

          <aside className="space-y-6 xl:sticky xl:top-5 xl:self-start">
            <section className="rounded-2xl border border-amber-300/30 bg-stone-900 p-5"><p className="text-xs font-semibold tracking-[0.18em] text-stone-500">当前价格</p><p className="mt-2 break-all font-mono text-3xl font-semibold tracking-tight text-amber-200 sm:text-4xl">{formatGameMoney(currentRound?.current_price ?? currentLot.starting_price)}</p><p className="mt-3 text-sm text-stone-400">当前最高 Owner</p><p className="mt-1 break-words text-xl font-semibold text-stone-100">{snapshot.bids.find((bid) => bid.owner?.id === currentRound?.current_winner_owner_id)?.owner?.display_name ?? (currentRound?.current_winner_owner_id ? "当前领先 Owner" : "等待首笔报价")}</p><div className="mt-5"><AuctionCountdown deadline={deadline} mode={currentRound?.status === "BIDDING" ? "BIDDING" : currentRound?.status === "OPEN_WAITING" ? "OPEN_WAITING" : null} /></div></section>
