...
import { PostRaceHealthForm } from "@/components/post-race-health-form";
...
function ResultFacts({ result, actualRace }: { result: RaceResultHistoryItem; actualRace: RaceResultsWorkbenchProps["actualRace"] }) {
  return <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-4">
    ...
    {(result.wpYear !== actualRace.wpYear || result.wpMonth !== actualRace.wpMonth || result.wpWeek !== actualRace.wpWeek) && <p className="sm:col-span-2 xl:col-span-4 rounded border border-amber-300/30 bg-amber-300/5 px-3 py-2 text-xs leading-5 text-amber-100">这条赛果当前关联到另一场实际比赛；赛前计划仅用于对比。</p>}
  </div>;
}
    ...
    <div className="space-y-8">
      <section>
        <div className="flex flex-col gap-3 border-b border-stone-800 pb-4 sm:flex-row sm:items-end sm:justify-between">
          <div><h2 className="text-xl font-semibold text-amber-200">候选已确认赛程</h2><p className="mt-2 text-sm leading-6 text-stone-400">系统只按时间和比赛事实推荐，不会自动绑定。已存在有效赛果的赛程已排除；已作废历史不会阻止重新录入。</p></div>
      ...
      <section className="border-t border-stone-800 pt-8"><h2 className="text-xl font-semibold text-stone-300">已作废历史</h2><p className="mt-2 text-sm text-stone-500">作废记录仅 GM 可见，不会出现在公开赛果或马匹统计中。</p><div className="mt-5 space-y-4">{voidedResults.map((result) => <article className="rounded-xl border border-stone-800 bg-stone-950/60 p-5" key={result.resultId}><div className="flex flex-col gap-3 sm:flex-row sm:justify-between"><div><p className="font-semibold text-stone-200">{result.horseName} <span className="font-mono text-xs text-stone-500">#{result.horseNumber}</span></p><p className="mt-1 text-sm text-stone-500">马主：{result.ownerName} · {formatRaceResultStatus(result.status)}</p></div><p className="text-sm text-stone-500">作废于 {formatDateTime(result.voidedAt)}</p></div><ResultFacts actualRace={actualRace} result={result} /><p className="mt-4 rounded border border-red-400/25 bg-red-400/5 p-3 text-sm leading-6 text-red-100">作废原因：{result.voidReason || "—"}</p></article>)}{!voidedResults.length && <p className="rounded-xl border border-stone-800 bg-stone-900 p-6 text-sm text-stone-500">暂无 已作废历史。</p>}</div></section>
    </div>
  );
}
...
function ResultCard({ result, actualRace, actualRaces }: { result: RaceResultHistoryItem; actualRace: RaceResultsWorkbenchProps["actualRace"]; actualRaces: ResultActualRaceOption[] }) {
  ...
  return <article className="rounded-xl border border-emerald-400/30 bg-emerald-400/5 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-semibold text-stone-100">{result.horseName} <span className="font-mono text-xs text-stone-500">#{result.horseNumber}</span></p><p className="mt-1 text-sm text-stone-400">马主：{result.ownerName} · {result.source}</p></div><span className="w-fit rounded-full border border-emerald-400/40 px-3 py-1 text-xs font-semibold text-emerald-100">{formatRaceResultStatus(result.status)}</span></div><div className="mt-4 grid gap-3 rounded-lg border border-stone-800 bg-stone-950/50 p-4 text-sm sm:grid-cols-2"><div><p className="text-xs text-stone-500">赛前计划</p><p className="mt-1 text-stone-200">{formatWpTime(result.wpYear, result.wpMonth, result.wpWeek)} · {planRaceName(result)}</p><p className="mt-1 text-stone-400">骑手：{result.jockey || "未指定"} · 跑法：{result.runningStyle || "未指定"}</p></div><div><p className="text-xs text-stone-500">实际比赛</p><p className="mt-1 text-amber-100">{formatWpTime(actualRace.wpYear, actualRace.wpMonth, actualRace.wpWeek)} · {actualRace.raceName} {actualRace.grade ? formatRaceGrade(actualRace.grade) : ""}</p></div></div><ResultFacts actualRace={actualRace} result={result} />{result.gmNote && <p className="mt-4 rounded border border-stone-700 bg-stone-950/60 p-3 text-sm leading-6 text-stone-300">GM 备注：{result.gmNote}</p>}<PostRaceHealthForm actualRaceId={actualRace.id} currentStamina={result.currentStamina} defaultWp={{ year: actualRace.wpYear, month: actualRace.wpMonth, week: actualRace.wpWeek }} hasLaterActiveHealthEvent={result.hasLaterActiveHealthEvent} horseId={result.horseId} isProcessed={result.postRaceProcessed} raceResultId={result.resultId} /><div className="mt-5 grid gap-4 xl:grid-cols-2"><details className="rounded-lg border border-amber-300/25 bg-stone-950/40 p-4"><summary className="cursor-pointer text-sm font-semibold text-amber-100">纠错当前赛果</summary><p className="mt-2 text-xs leading-5 text-stone-400">只能修改实际比赛和赛果事实；不能修改马匹或已确认赛程。请在确认前核对修改前后内容并填写原因。</p><ActionForm action={correct} className="mt-4 space-y-3" confirmation="确认纠错这条赛果吗？公开页面将只显示纠错后的当前事实。" pendingLabel="正在纠错…" submitLabel="确认纠错"><label className="admin-label">目标实际比赛<select className="admin-input" defaultValue={result.actualRaceId} name="actual_race_id">{actualRaces.map((race) => <option key={race.id} value={race.id}>{formatWpTime(race.wp_year, race.wp_month, race.wp_week)} · {race.race_name} {race.grade ? formatRaceGrade(race.grade) : ""}</option>)}</select></label><div className="grid gap-3 sm:grid-cols-2"><label className="admin-label">名次<input className="admin-input" defaultValue={result.finishPosition} min="1" max="99" name="finish_position" required type="number" /></label><label className="admin-label">Winning Post 实际赏金<input className="admin-input" defaultValue={result.prizeAmount} inputMode="numeric" name="prize_amount" required type="text" /></label></div><label className="admin-label">实际骑手（可选）<input className="admin-input" defaultValue={result.actualJockey ?? ""} name="actual_jockey" /></label><label className="admin-label">实际跑法（可选）<select className="admin-input" defaultValue={result.actualRunningStyle ?? ""} name="actual_running_style"><option value="">未指定</option><option value="逃">逃</option><option value="先">先</option><option value="差">差</option><option value="追">追</option></select></label><label className="admin-label">GM 备注（仅 GM）<textarea className="admin-input min-h-20" defaultValue={result.gmNote ?? ""} name="gm_note" /></label><label className="admin-label">纠错原因<textarea className="admin-input min-h-20" name="reason" required /></label></ActionForm></details><details className="rounded-lg border border-red-400/30 bg-red-400/5 p-4"><summary className="cursor-pointer text-sm font-semibold text-red-100">作废赛果</summary><p className="mt-2 text-xs leading-5 text-stone-300">作废不会删除历史记录。{result.postRaceProcessed ? "它还会作废该赛果的赛后健康事件、回退体力，并作废由该事件产生的伤病。" : "该赛果会从公开有效赛果中移除，之后可重新录入新的有效结果。"}</p>{result.hasLaterActiveHealthEvent && <p className="mt-3 rounded border border-amber-300/30 bg-amber-300/5 p-3 text-xs leading-5 text-amber-100">当前赛后事件不是最新有效健康事件；数据库会拒绝不安全的赛果作废。请先在马匹健康历史中处理之后的事件。</p>}<ActionForm action={voidResult} className="mt-4 space-y-3" confirmation="第二次确认：确定作废这条赛果吗？如已有赛后状态，会同步回退体力与伤病历史。" pendingLabel="正在作废…" submitLabel="作废赛果" variant="danger"><label className="admin-label">作废原因<textarea className="admin-input min-h-20" name="reason" required /></label></ActionForm></details></div></article>;
}
