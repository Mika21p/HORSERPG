...
GM 可在单一 Owner 的管理详情中进行受控的指定资金调整。该操作只能新增固定类别 `GM_MANUAL_ADJUSTMENT` 的正/负正式流水，必须填写非空原因、使用请求键幂等，并写入 Audit Log；不得编辑 `initial_funds`、既有流水或冻结记录。数据库会在同一事务重新计算庭先有效秘密报价与公开拍卖当前赢家报价形成的冻结，拒绝任何会使调整后的账户资金或可用资金为负的请求。
